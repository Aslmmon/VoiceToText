package com.idmt.simplevoice.recognition
...
class VoiceTextParser(
    ...
    fun startListening(languages: String = ("en-US")) {
        ...
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
            ...
            putExtra(RecognizerIntent.EXTRA_LANGUAGE, languages)
    ...
    override fun onEndOfSpeech() {
        _state.update {
            it.copy(isSpeaking = false, isSpoken = "Analyzing ..")
        }
    }
    ...
    override fun onResults(results: Bundle?) {
        results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.getOrNull(0)
            ?.let { result ->
                _state.update {
                    it.copy(isSpoken = result, appendText = true)
                }
            }
    }
